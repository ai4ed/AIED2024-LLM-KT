...
ALL_KEYS = ["fold", "uid", "questions", "concepts", "responses", "timestamps", "usetimes", "selectmasks", "is_repeat", "qidxs", "rest", "orirow","cidxs", "dataset"]

def get_pretrain_data(seqlen, data_config, train_ratio=1.0):
    ...
    if train_ratio < 1.0:
        sub_data_path = os.path.join(data_config["dpath"], f"train_valid_quelevel_pretrain_{train_ratio}.csv")
    ...
    split_seqs = generate_sequences(df=finaldf, effective_keys={"uid","questions","concepts","responses", "fold", "timestamps", "dataset"}, min_seq_len=3, maxlen=seqlen)
    dpath = data_config["dpath"]
    if train_ratio < 1.0:
        split_seqs.to_csv(f"{dpath}/train_valid_sequences_quelevel_{seqlen}_{train_ratio}.csv", index=None)
    else:
        split_seqs.to_csv(f"{dpath}/train_valid_sequences_quelevel_{seqlen}_ablation.csv", index=None)
...
def extract_sub_pretrain_data(finaldf, train_ratio, datasets):
    ...
    for data in datasets:
        sub_df = finaldf[finaldf.dataset==data].sample(frac=train_ratio, random_state=1024)
        final_sub_df = pd.concat([final_sub_df,sub_df],ignore_index=True)
    print(f"extract_sub_pretrain_data...original_stu_nums:{finaldf.shape}, extract_nums:{final_sub_df.shape}")
    return final_sub_df
