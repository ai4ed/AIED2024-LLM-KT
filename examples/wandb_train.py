...
def main(params, args=None):
    ...
    model_name, dataset_name, fold, emb_type, save_dir, global_bs, num_workers, num_gpus = (
        params["model_name"],
        params["dataset_name"],
        params["fold"],
        params["emb_type"],
    ...
    with open("../configs/kt_config.json") as f:
        ...
        if model_name in ["gpt4kt","spkt"]:
            ...
            if seqlen == 1024:
                ...
                train_config["batch_size"] =int(global_bs / num_gpus / num_workers)
    ...
    params_str = "_".join(
        [str(v) for k, v in params.items() if not k in ["other_config","pretrain_path","pretrain_epoch"]]
    )
    ...
    if save_model:
        ...
        if ckpt_path is not None:
            try:
                ...
                net = torch.load(os.path.join(ckpt_path, emb_type + "_model.module_{}.ckpt".format(best_epoch)))
                best_model.load_state_dict(net)
    ...
    if ckpt_path is not None:
        model_save_path = os.path.join(ckpt_path, f"{emb_type}_model.module_{best_epoch}.ckpt")
    else:
